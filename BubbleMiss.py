    ...
    if gameover == False:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:  ## Key down moves cannon down
                ...
                ):  ##space shoots bullets through the cannon with 1.5 times the velocity
                    if fired == False:
                        bullet_state = True
                        buy = cy + 40
                        bux = cx - 50
            ...
            ):  ##when user leaves the key the cannon should stop moving
        ...
        if (
            math.sqrt(
                math.pow(bx - (bux + 25 / 2), 2) + math.pow(by - (buy + 25 / 2), 2)
            )
            < 50
        ):
            bullet_state = False
        ...
        textsurface1 = myfont.render(
            "Bullets Missed: " + str(counter), False, (255, 0, 0)
        )
